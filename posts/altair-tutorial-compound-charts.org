...
The last column is the species so we'll grab all the ones before it. Note that the default columns object is an Index, so you have to cast it to a list or an array.
